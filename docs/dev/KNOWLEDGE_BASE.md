...
| **有效配置目录** | 读写的配置根目录：优先 `<项目根>/config/`，否则 `~/.mif/`。 |

---

## 二、配置文件与路径一览

所有路径均通过 `mif.config` 解析，**项目 config 优先，用户目录兜底**。

| 文件 | 路径（优先级从高到低） | 用途 | 写入方 |
|------|------------------------|------|--------|
| 工作流 | `config/workflows.json` → `workflows.json`(根) → `~/.mif/workflows.json` | 工作流条目（id/name/action/args/keywords/icon/priority） | 用户/脚本 |
| 插件配置 | `config/plugin_configs.json` → `~/.mif/plugin_configs.json` | 插件参数与 `_keywords`/`_at_keyword` 覆盖 | PluginManager、@settings |
| Chrome 书签点击 | `config/chrome_bookmarks_clicks.json` → `~/.mif/chrome_bookmarks_clicks.json` | URL → 点击次数，用于书签排序 | ChromeBookmarksPlugin |
...
- **PluginManager**（`mif.plugins`）
  - 启动时扫描 `mif/plugins/*.py`（除基础与内部模块），自动注册 `BasePlugin` 子类。
  - 当前内部已拆分为 `registry` / `config_store` / `coordinator`，对外 API 保持兼容。
...
- **WorkflowItem**（`mif.workflow`）：id、name、description、action、args、icon、priority、keywords。`run(query)` 执行时会将 args 中的 `{query}` 替换为传入的 query 再调用 ActionRegistry。
...
- **WorkflowPlugin**（`mif.plugins.workflow_plugin`）：内置插件，读取 `workflows.json`，将每条工作流转为 PluginResult；支持每条的关键词前缀匹配与模糊搜索（thefuzz）；`@wf` 可浏览/筛选工作流。工作流结果 `action_args=(subst_query,)` 供 `run(query)` 使用。
...
- **当前实现**：`mif/gui_qt/launcher.py`（Qt Widgets）
- **分层调用**：GUI -> `ApplicationService` -> `PluginManager`
- **执行反馈**：支持底部短时横幅（成功/失败）
...
| 职责 | 文件 |
|------|------|
| 配置路径与工作流加载 | `mif/config.py` |
| 工作流定义与动作注册 | `mif/workflow.py` |
| 插件基类与结果类型 | `mif/plugins/base.py` |
| 插件管理外观层 | `mif/plugins/__init__.py` |
| 插件发现/配置/搜索拆分 | `mif/plugins/registry.py` / `config_store.py` / `coordinator.py` |
| 应用服务层 | `mif/application/service.py` |
| Qt GUI 主界面 | `mif/gui_qt/launcher.py` |
